...
What went wrong
Ensemble launched into a space already occupied by Poe, TeamAI, TypingMind, and others. Users didn't switch because the alternatives were good enough.

Why it happened
We built a feature (multi-LLM comparison) that competitors already offer. We didn't clearly define what makes Ensemble different, and couldn't answer "why not just use Poe?" in a convincing way.

How we prevent it
Ensemble is not a comparison tool. It is a synthesis workspace.
Existing tools stop at "compare and pick one." Ensemble goes further — users take answers from multiple models and actively combine, edit, and build from them. The core interaction is not selection, it's construction.
Additionally, the onboarding experience — where users articulate how they actually use each LLM — is a differentiator no competitor currently offers. It makes Ensemble feel personal from the first minute.
The answer to "why not just use Poe?" is: Poe gives you four answers. Ensemble helps you make one better one.



Risk 2 — The team couldn't agree on what Ensemble is for

What went wrong
Five people agreed on the core features but had completely different visions for what Ensemble should ultimately do — and for whom. The project stalled in endless debate and never found a clear direction.
...
•	What Ensemble believes (MANIFEST)
•	Why Ensemble exists, all the way up (WHYTREE)
...
How we prevent it
Three responses:
First, scope ruthlessly. Ensemble MVP is three features: multi-send, side-by-side view, and basic mixing. Nothing else ships in v1.
...
Risk 4 — The target user was too narrow

What went wrong
Almost no one needed Ensemble. Most people use one or two LLMs casually and never feel the friction we were solving. The product was built for a niche that wasn't large enough to matter.
...
Ensemble should be exactly right for a small number of people, not roughly okay for everyone.



Risk 5 — The use case was too vague

What went wrong
Ensemble positioned itself as useful for "any question," but that breadth made it forgettable. Users couldn't identify a specific moment where Ensemble was the obvious tool to reach for, so they didn't.

Why it happened
The team focused on what Ensemble could do rather than when and why someone would open it. Without a concrete trigger — a specific task, context, or pain point — the product felt generic despite its differentiated mechanics.

How we prevent it
Define three concrete trigger scenarios: the moments where a power user would naturally think "this is exactly when I need Ensemble." Examples: drafting a high-stakes document, stress-testing a decision, or synthesizing research across domains.
Anchor all onboarding, copy, and UX flows to these scenarios. Every new user should be able to say, within two minutes, exactly when they'll use Ensemble again.
If a proposed feature doesn't map to one of the three scenarios, it doesn't ship in v1.



Risk 6 — API costs were higher than expected
...
Risk 7 — API access and rate limits became a bottleneck

What went wrong
One of our core LLM providers changed its pricing, deprecated a model we relied on, or aggressively throttled usage as our volume grew. A key part of the user experience broke overnight — and we had no fallback.
...
Maintain at least one fallback option for each core model capability. If GPT-4o becomes unavailable, Ensemble degrades gracefully to Claude or Gemini for that task type.
...
Risk 8 — The comparison UI caused decision fatigue

What went wrong
Users opened Ensemble, saw three different outputs from three different models, and froze. Instead of feeling helped, they felt overwhelmed. The tool that was supposed to reduce cognitive load added to it. Users churned or reverted to single-model tools.
...
Competitors already exist	Feature overlap, no clear differentiation	Synthesis vs. comparison — we build, not pick
Team misalignment	No shared "why," only shared "what"	MANIFEST + WHYTREE as decision foundation
Technical overload	1 tech member, overloaded	Ruthless scope + Claude Code for whole team
Market too narrow	Built for ourselves, assumed universal pain	Own the niche, don't dilute for mass market
Use case too vague	No clear scenario for when/why users reach for Ensemble	Define 3 concrete trigger moments; anchor all UX to them
API costs higher than expected	Routing overhead + user habit of selecting all models	Default to top-1 model; show cost per query; set soft caps
API access & rate limits	Dependency on third-party LLM providers	Abstract provider layer; maintain fallback routing per model
Comparison UI causes decision fatigue	Too many outputs, no clear signal on which is better	Separate "quick compare" and "think mode"; reduce default output count



PREMORTEM — Ensemble (한국어)
...
무엇이 잘못되었나
Ensemble는 Poe, TeamAI, TypingMind 등이 이미 자리잡은 시장에 출시되었다. 유저들은 기존 대안으로 충분했기 때문에 갈아타지 않았다.

왜 그렇게 되었나
경쟁자들이 이미 제공하는 기능(멀티 LLM 비교)을 만들었다. Ensemble가 무엇이 다른지 명확히 정의하지 못했고, "그냥 Poe 쓰면 되지 않나요?"라는 질문에 설득력 있게 답하지 못했다.

어떻게 막을 것인가
Ensemble는 비교 툴이 아니다. 합성 워크스페이스다.
기존 툴은 "비교하고 하나를 고른다"에서 멈춘다. Ensemble는 더 나아간다 — 여러 모델의 답변을 재료로 삼아 능동적으로 조합하고, 편집하고, 구축한다. 핵심 인터랙션은 선택이 아니라 구성이다.
또한, 유저가 각 LLM을 실제로 어떻게 사용하는지 스스로 인식하게 하는 온보딩 경험은 현재 어떤 경쟁자도 제공하지 않는 차별점이다.
"그냥 Poe 쓰면 되지 않나요?"에 대한 답: Poe는 네 개의 답을 준다. Ensemble는 더 나은 하나를 만들도록 돕는다.



리스크 2 — 팀이 Ensemble의 방향에 합의하지 못했다

무엇이 잘못되었나
다섯 명이 핵심 기능에는 동의했지만, Ensemble가 궁극적으로 무엇을 해야 하는지에 대해 완전히 다른 비전을 가지고 있었다. 프로젝트는 끝없는 논쟁 속에서 방향을 잡지 못하고 표류했다.
...
•	Ensemble가 믿는 것 (MANIFEST)
•	Ensemble가 존재하는 이유, 끝까지 (WHYTREE)
•	Ensemble를 죽일 수 있는 것, 그리고 대응 (PREMORTEM)
...
어떻게 막을 것인가
세 가지 대응:
첫째, 스코프를 철저히 좁힌다. Ensemble MVP는 세 가지 기능뿐이다: 멀티-send, 나란히 보기, 기본 믹싱. v1에서는 그 외 아무것도 출시하지 않는다.
둘째, Claude Code를 진정한 equalizer로 사용한다 — 기술 멤버만의 도구가 아니라, 팀 전체가 각자의 영역에서 활용하는 도구로. 비기술 멤버도 카피, 온보딩 플로우, UX 결정을 소유하고 Claude Code로 구현한다.
셋째, 기술 멤버의 역할을 "만드는 사람"에서 "구조를 정의하는 사람"으로 전환한다. 구조는 그가 잡고, 팀이 AI의 도움으로 그 안에서 만든다.
...
무엇이 잘못되었나
거의 아무도 Ensemble를 필요로 하지 않았다. 대부분의 사람들은 LLM 한두 개를 가볍게 쓰고 우리가 해결하려는 마찰을 느끼지 않았다. 제품은 충분히 크지 않은 니치를 위해 만들어졌다.
...
대신, 니치에 더 깊이 집중한다. 세 개 이상의 LLM을 정기적으로 쓰고, 여러 구독료를 내고, 어떤 모델을 어떤 용도로 쓸지 능동적으로 생각하는 사람 — 그 사람은 존재하고, 그 수는 늘고 있고, 현재 툴로는 완전히 서비스받지 못하고 있다.
...
Ensemble는 소수의 사람들에게 정확히 맞아야 한다. 모두에게 대충 괜찮으면 안 된다.



리스크 5 — 사용 시나리오가 너무 애매했다

무엇이 잘못되었나
Ensemble는 "어떤 질문에도 유용하다"고 포지셔닝했지만, 오히려 그 범용성이 존재감을 흐렸다. 유저들은 Ensemble를 꺼내야 할 구체적인 순간을 떠올리지 못했고, 결국 쓰지 않았다.

왜 그렇게 되었나
팀은 Ensemble가 무엇을 할 수 있는지에 집중했고, 언제 그리고 왜 누군가가 Ensemble를 여는지는 정의하지 않았다. 구체적인 트리거 — 특정 작업, 맥락, 고통의 순간 — 없이 제품은 차별화된 메커니즘에도 불구하고 평범하게 느껴졌다.

어떻게 막을 것인가
구체적인 트리거 시나리오 세 가지를 정의한다. 파워유저가 자연스럽게 "지금이 바로 Ensemble가 필요한 순간"이라고 생각하게 되는 상황들이다. 예시: 중요한 문서 초안 작성, 의사결정 검증, 여러 분야의 리서치 합성.
모든 온보딩, 카피, UX 플로우를 이 시나리오에 맞춘다. 모든 신규 유저가 2분 안에 언제 다시 Ensemble를 쓸지 말할 수 있어야 한다.
제안된 기능이 세 가지 시나리오 중 하나에 대응하지 않는다면 v1에는 들어가지 않는다.



리스크 6 — API 비용이 예상보다 높았다
...
리스크 7 — API 접근 및 속도 제한이 병목이 되었다

무엇이 잘못되었나
핵심 LLM 제공업체 중 하나가 가격 정책을 바꾸거나, 우리가 의존하던 모델을 deprecated 하거나, 사용량이 늘면서 공격적으로 속도 제한을 걸었다. 핵심 유저 경험이 하룻밤 사이에 깨졌고, 대안이 없었다.

왜 그렇게 되었나
추상화 레이어 없이 특정 제공업체 API에 직접 의존하는 구조로 만들었다. 우리가 통제할 수 없는 서드파티 서비스에 대한 안정적이고 중단 없는 접근을 당연시했다.

어떻게 막을 것인가
처음부터 제공업체 추상화 레이어를 만든다. 모든 모델 호출은 직접 API 호출이 아닌 내부 라우팅 인터페이스를 통한다. 이렇게 하면 제공업체 교체나 다운그레이드가 리라이트가 아닌 설정 변경으로 가능해진다.
핵심 모델 기능별로 최소 하나의 폴백 옵션을 유지한다. GPT-4o를 쓸 수 없게 되면, 해당 작업 유형에 대해 Claude나 Gemini로 graceful하게 다운그레이드된다.
...
리스크 8 — 비교 UI가 결정 피로도를 만들었다

무엇이 잘못되었나
유저들이 Ensemble를 열고 세 모델의 서로 다른 출력을 마주하자 얼어붙었다. 도움을 받는다는 느낌 대신 압도감을 느꼈다. 인지 부하를 줄이려던 툴이 오히려 부하를 더했다. 유저들은 이탈하거나 단일 모델 툴로 돌아갔다.
...
리스크	근본 원인	대응 방안
경쟁자 이미 존재	기능 중복, 차별화 불명확	합성 vs. 비교 — 고르는 게 아니라 만든다
팀 방향성 충돌	공통 "왜" 없이 "무엇"만 합의	MANIFEST + WHYTREE를 의사결정 기반으로
기술 과부하	기술 멤버 1명, 과도한 책임	스코프 축소 + 팀 전체의 Claude Code 활용
시장이 너무 좁음	나를 위해 만들고 보편적 고통이라 가정	니치를 희석하지 않고 더 깊이 집중
사용 시나리오가 애매함	언제/왜 Ensemble를 쓰는지 구체적 상황이 없음	핵심 트리거 시나리오 3가지 정의; 모든 UX를 그 기준에 맞춤
API 비용이 예상보다 높음	라우팅 오버헤드 + 사용자가 습관적으로 전체 모델 선택	기본값은 상위 1개 모델; 쿼리당 비용 표시; 소프트 상한 설정
API 접근 및 속도 제한	제3자 LLM 제공업체 의존	제공업체 추상화 레이어 구축; 모델별 폴백 라우팅 유지
비교 UI로 인한 결정 피로도	출력물이 너무 많고 어느 것이 더 나은지 신호가 없음	"빠른 비교"와 "사고 모드" 분리; 기본 출력 수 줄이기
